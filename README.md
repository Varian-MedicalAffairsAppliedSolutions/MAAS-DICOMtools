...
## New in V1.2.1
- **Selective Plan Retrieval**: When using `--planId` or `--onlyApprovedPlans`, the retrieve command now filters RT-Plans FIRST before downloading data, dramatically reducing unnecessary data transfer for patients with many plans. See `RETRIEVE_REFACTORING.md` for details.
...
## Retrieve
    ...
    Options:
      ...
      --courseId <courseId>                 Id of the course to retrieve data.
...
### Selective Plan Retrieval (New in V1.2.1)
When **--courseId**, **--planId**, or **--onlyApprovedPlans** is specified, the retrieve command uses **selective retrieval**:
1. Queries and retrieves **only RT-Plan instances** first
2. Parses and filters plans based on criteria (course ID, plan ID, approval status)
3. Retrieves **only the data referenced** by matching plans (structure sets, doses, images, etc.)

This dramatically reduces data transfer for patients with many plans! The filtering hierarchy is: Patients → Courses → Plans.
Without plan filters, all patient data is retrieved (original behavior).
