...
(function(){
  ...
  function buildCommandLine(operation, parameters) {
    ...
    switch (operation) {
      case 'retrieve':
        ...
        // Add showTree by default for debugging
        args.push('--showTree');
  ...
  async function createBatchFile(operation, parameters) {
    ...
    if (operation === 'retrieve' && isMultiPatient) {
      ...
      patientIdsArray.forEach(pid => {
        const sanitizedPid = pid.replace(/[<>:\"\\/\\|?*]/g, '_').trim();
        const subPath = `${parameters.path}\\${sanitizedPid}`;
        retrieveExecutionBlock += `echo [INFO] Retrieving patient ${pid}\r\n`;
        ...
        retrieveExecutionBlock += `    timeout /t 1 /nobreak >nul\r\n`;
        retrieveExecutionBlock += `)\r\n`;
        retrieveExecutionBlock += `"%_DT_EXE%" retrieve --patientId "${pid}" --path "${subPath}" --showTree ${hostArgs.join(' ')}\r\n\r\n`;
      });
    }
    if (operation === 'retrieve' && !isMultiPatient) {
      ...
      retrieveSingleExecutionBlock += `echo [INFO] Retrieving patient ${pid}\r\n`;
...
This is the most thorough diagnostic tool - use this for comprehensive troubleshooting!`);
  ...
  function tryLoadFromGlobal() {
    try {
      if (typeof window !== 'undefined' && window.PATIENT_IDS) {
        const ids = String(window.PATIENT_IDS || '').trim();
        if (ids) {
          const el = $('patientId');
          if (el) el.value = ids;
          showMessage(`Loaded ${ids.split(';').filter(x=>x.trim()).length} patient ID(s) from Patient List Builder.`);
          autoSaveForm();
          // Show multi-patient hint immediately; plans total may be unknown until JSON loads
          const patientIds = ids.split(';').map(p=>p.trim()).filter(Boolean);
          if (patientIds.length > 0) {
            // If handoff data is present (even if totals are 0), use it for accurate totals
            const hasHandoffData = (typeof window.HANDOFF_DATA === 'object' && window.HANDOFF_DATA !== null &&
              ('totalPatients' in window.HANDOFF_DATA || 'totalPlans' in window.HANDOFF_DATA || 'patients' in window.HANDOFF_DATA || 'Patients' in window.HANDOFF_DATA));
            if (hasHandoffData) {
              try {
                const json = window.HANDOFF_DATA;
                const extractedIds = extractPatientIds(json);
                updatePatientDisplayUI(extractedIds.length ? extractedIds : patientIds, json);
              } catch { updatePatientDisplayUI(patientIds, {}); }
            } else {
  ...
  async function tryLoadHandoffData() {
    // When launched via file://, fetching local files is blocked by CORS; rely on injected globals instead
    try {
    ...
    // Load patient IDs
    try {
      const idsResp = await fetch('handoff_patient_ids.txt', { cache: 'no-store' });
      if (idsResp && idsResp.ok) {
        const idsText = (await idsResp.text() || '').trim();
        if (idsText) {
          const el = $('patientId');
          if (el) el.value = idsText;
          showMessage(`Loaded ${idsText.split(';').filter(x=>x.trim()).length} patient ID(s) from handoff.`);
          autoSaveForm();
        }
      }
    } catch (e) {
      // ignore - handoff not present
    }

    // Load full JSON (optional) for richer display
    try {
      ...
      if (jsonResp && jsonResp.ok) {
        ...
        if (ids && ids.length > 0) {
          updatePatientDisplayUI(ids, jsonData);
        }
      }
    } catch (e) {
      // ignore - handoff not present or file:// CORS
      try {
        if (window.HANDOFF_DATA || window.HANDOFF_FULL) {
          const json = window.HANDOFF_FULL || window.HANDOFF_DATA;
          const ids = extractPatientIds(json);
          if (ids && ids.length > 0) {
            updatePatientDisplayUI(ids, json);
          }
        }
      } catch {}
    }
